...
location = "/home/gsalinas/GitHub/MyTransport" # This should be the location of the PyTransport folder folder
sys.path.append(location + '/PyTransport') # Sets up python path to give access to PyTransSetup
...
import PyTransAngularPolar as PyT
import PyTransScripts as PyS

with open(location_model + '/ModelSetup/G.txt','rb') as f:
    G = pickle.load(f)
with open(location_model + '/ModelSetup/Gammas.txt','rb') as f:
    Gammas = pickle.load(f)
with open(location_model + '/ModelSetup/V.txt','rb') as f:
    V = pickle.load(f)
...
param_vals = np.array([1/600, 9, 2e-5]) # Parameter values, format [alpha, R, mphi]
r0, theta0 = 0.99, pi/4
fields0 = r0 * np.array([cos(theta0), sin(theta0)]) # Initial values of the fields [phi, chi]

V = PyT.V(fields0, param_vals) # Calculate potential from some initial conditions
dV = PyT.dV(fields0, param_vals) # Calculate derivatives of potential

fielddots0 = np.zeros(2) # set initial conditions for the field velocities
initial = np.concatenate((fields0, fielddots0)) # Sets an array containing field values and there derivative in cosmic time 

################################## Run and plot the background fiducial run ################################################################
print('Starting background evolution.')

Nstart, Nend = 0., 10
Ns = np.linspace(Nstart, Nend, 1_000)

tols = np.array([10**-6, 10**-6])
back = PyT.backEvolve(Ns, initial, param_vals, tols, False)
print(back)
...
psis = sqrt(6*param_vals[0]) * np.arctanh(back[:, 1])
plt.plot(psis * np.cos(back[:, 1]), psis * np.sin(back[:, 1]))
plt.xlabel(r'$\psi \cos(\theta)$')
plt.ylabel(r'$\psi \sin(\theta)$')
plt.tight_layout()
plt.savefig(location_model + '/Output/Background.png')
plt.clf()    

# # Find Hubble rate
# Hs = np.array([PyT.H(elem, param_vals) for elem in back[:, 1:]])
# plt.plot(back[:, 0], Hs)
...
# plt.yscale('log')
# plt.tight_layout()
# plt.savefig(location_model + '/Output/Hubble.png')
# plt.clf()
...
# plt.yscale('log')
# plt.tight_layout()
# plt.savefig(location_model + '/Output/Epsilon.png')
# plt.clf()
# print("Average epsilon: ", np.mean(epsilon))

# print(back[:, 3:5])

# def sigma(back: np.ndarray) -> np.ndarray:
#     return back[:, 3] / np.linalg.norm(1)

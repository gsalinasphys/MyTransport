...
# pval = np.array([3.4e-10, 2.13974e-14, 2.5e-3, 0.000017717]) # Parameter values, format [V0, alpha, m, rho0]
pval = np.array([5.9e-10, 1.54836e-13, 4.5e-4, 0.000683557])
...
phidot0 = np.array([0., pval[2]])    # Initial field velocities
...
Nstart, Nend = 0., 100
t = np.linspace(Nstart, Nend, 1_000_000)

tols = np.array([10**-4, 10**-4])
...
plt.savefig('Examples/RapidTurn/Hubble.png')
plt.clf()
...
plt.savefig('Examples/RapidTurn/Epsilon.png')
plt.clf()
print("Initial epsilon: ", epsilon[0])

# Turn rate
def turn_rate(hat_rhos: np.ndarray, hat_thetas: np.ndarray, dt: float) -> float:
    return np.sum(hat_rhos * np.gradient(hat_thetas, dt, axis=0), axis=1)
    
hat_rhos = np.array([[cos(theta), sin(theta)] for theta in back[:, 2]])
hat_thetas = np.array([[-sin(theta), cos(theta)] for theta in back[:, 2]])

plt.plot(back[:, 0], turn_rate(hat_rhos, hat_thetas, dt))
plt.title('Turn rate')
plt.xlabel(r'$N$', fontsize=16)
plt.ylabel(r'$\omega$', fontsize=16)
plt.savefig('Examples/RapidTurn/TurnRate.png')
plt.clf()
...
plt.savefig('Examples/RapidTurn/DTurnRate.png')
plt.clf()
...
talp = np.linspace(Nstart, Nend, 1_000)

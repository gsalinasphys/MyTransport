...
V = p[0] - p[1]*f[1] + 0.5*p[2]**2*(f[0] - p[3])**2   # this is the potential written in sympy notation
G = sym.Matrix([[1, 0], [0, f[0]**2]]) # this is the field metric written in sympy notation

PyTransSetup.potential(V,nF,nP,False,G) # writes this potential and its derivatives into C++ file potential.h when run

PyTransSetup.compileName3("Rapid",True) # this compiles a python module using the C++ code, including the edited potential.h file, called PyTransRapid
